...
def enqueue(queue, num_nodes, scheduler, tracefile, comment):
    m = Message()
    s = json.dumps({
            'scheduler': scheduler,
...
def queue_sweep(args):
    ...
    for scheduler in schedulers:
        for num_nodes in node_cts:
            enqueue(queue, num_nodes, scheduler, tracefile, comment)


def usage():
    print 'Usage: queue_sweep.py min_nodes max_nodes node_step_mult schedulers comment tracefile'
    print '    e.g., queue_sweep.py 1 16 2 "trivial location_aware" "just testing" trace.json'
