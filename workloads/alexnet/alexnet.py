...
def load_tarfile_from_s3(s3_key, size=[]):
...
def load_tarfiles_from_s3(repetitions, s3_keys, size=[]):
  ...
  #return [load_tarfile_from_s3.remote(s3_key, size) for s3_key in s3_keys for k in range(repetitions)]
  return [load_tarfile_from_s3(s3_key, size) for s3_key in s3_keys for k in range(repetitions)]
