...
do
  ...
  do
    for s in `seq 1 5`
    do
       echo running ray_rnn with $k workers $n steps and scale $((s*5))
       #python rnn_ray_loop.py -w $k -s $s
       python rnn_ray_loop.py -w $k -s $s -n $n | tail -n2 | head -n1 | paste -sd ',' >> $timing_output_file 
    done
  done
done
